...
Features, CONTEXT.md, and CONTEXT-MAP.md: `.toolboxes/bdd_toolbox/`. Brainstorm: `.toolboxes/bdd_toolbox/brainstorm.txt` — read once as intake, grill path only; do not write to it.

## Process

1. Classify against the table, top row first. Stop, simple-edit, or confirm rows: do that and stop. Confirm: persist just-accepted terms if CONTEXT.md is missing them, then Confirm Shared Understanding — no grill, no Features.
...
3. Grill path only: first user-visible text is one line of slice scope, then numbered questions. Nothing else. Read Brainstorm once if present; still ask those branches. Ask only the unblocked, unsettled frontier (prior question, prior round, or accepted recommendation is settled). The recommended answer must be the same rule as your private conclusion. Do not write `.feature` files.
...
5. Load `gherkin-authoring` only from its table row. Follow it with the path override above. After confirm, leftover branches are out of scope — do not Feature them, do not load grill.
...
User talks implementation → remind: design only, not implementation. If blocked, ask one clarifying question. Do not invent Features this turn did not ask for.

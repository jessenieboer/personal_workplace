...
(defvar my-hidden-buffer-regexp nil)
(defvar my-tab-cache-timer nil)

(defun my-rebuild-hidden-buffer-regexp ()
  (setq my-hidden-buffer-regexp
        (mapconcat #'identity my-hidden-buffer-patterns "\\|")))

(defun my-add-hidden-buffer-patterns (regex-list)
  (setq my-hidden-buffer-patterns
        (append my-hidden-buffer-patterns regex-list))
  (my-rebuild-hidden-buffer-regexp))
    ...
    (when buf
      (or (gethash buf my-project-name-cache)
          (with-current-buffer buf
            (let* ((dir (or (and (buffer-file-name buf)
                                 (file-name-directory (buffer-file-name buf)))
                            default-directory))
                   (name (if (or (not dir) (file-remote-p dir))
                             "no_project"
                           (if-let ((proj (project-current nil dir)))
                               (project-name proj)
                             "no_project"))))
              (puthash buf name my-project-name-cache)
              name))))))

(defun my-cache-for-tab-display (buffer)
  (when-let ((bname (buffer-name buffer)))
    (puthash bname
             (if (and my-hidden-buffer-regexp
                      (string-match-p my-hidden-buffer-regexp bname))
                 "Hidden"
               (my-get-project-name-for-buffer buffer))
             my-tab-display-cache)))

(defun my-tab-line-tabs-buffer-group-function (buffer)
  (let ((bname (buffer-name buffer)))
    (or (gethash bname my-tab-display-cache)
        (progn
          (my-cache-for-tab-display buffer)
          (gethash bname my-tab-display-cache "No tab group")))))

(defun my-invalidate-tab-cache-for-buffer (&optional buffer)
  (let ((buf (or buffer (current-buffer))))
    (remhash buf my-project-name-cache)
    (when-let ((bname (buffer-name buf)))
      (remhash bname my-tab-display-cache))))

(defun my-update-tab-display-cache ()
  "Idle rebuild of entries that are actually missing. Not a full scan."
  (setq my-tab-cache-timer nil)
  (let ((n 0))
    (dolist (buf (buffer-list))
      (when (and (< n 20)
                 (not (gethash (buffer-name buf) my-tab-display-cache)))
        (my-cache-for-tab-display buf)
        (setq n (1+ n))))))

(defun my-schedule-tab-cache-update (&rest _)
  (unless my-tab-cache-timer
    (setq my-tab-cache-timer
          (run-with-idle-timer 0.25 nil #'my-update-tab-display-cache))))

;; NOT buffer-list-update-hook
(add-hook 'find-file-hook #'my-invalidate-tab-cache-for-buffer)
(add-hook 'after-change-major-mode-hook #'my-invalidate-tab-cache-for-buffer)
(add-hook 'kill-buffer-hook #'my-invalidate-tab-cache-for-buffer)
(add-hook 'project-remember-project-hook #'my-schedule-tab-cache-update)
...
(my-add-to-hydra 'dired-mode
                 ("Connection"
		  (("iq" ora-ediff-files "ediff files" :exit t))))
...
;; Forge: GitHub PRs/issues inside Magit.
;; Token comes from secretspec (Bitwarden via BWS), not pass:
;;   add GITHUB_FORGE_TOKEN in the Bitwarden vault secretspec reads,
;;   declared in home-manager secretspec.toml.
(require 'forge)
...
(my-add-to-hydra '(magit-diff-mode magit-status-mode)
		  ...
		  (
		   ;; setting up for work
  		   ...
  		   ("i DEL" magit-unstage "git unstage")
  		   ("i '" magit-reset "git reset" :exit t)
...
(with-eval-after-load 'lsp-mode
  (add-to-list 'lsp-disabled-clients 'racket-language-server)
  (lsp-register-client
   (make-lsp-client
